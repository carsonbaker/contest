...
module SIP
  ...
  class Session
    ...
    def self.start_track_sip_conv(sip_msg)
    ...
    def handle_invite(sip_msg : SIP::Msg, client_addr : Socket::IPAddress)
      ...
      # Otherwise... start tracking this as a new session
      call = SIP::Session.start_track_sip_conv(sip_msg)
      
      # Set up transport
      opus_port = sip_msg.sdp_header.rtp_map["opus/48000/2"]?
      if opus_port
        call.transport = Transport::RTP.new(call, Codec::Opus.new, opus_port)
      else
        call.transport = Transport::RTP.new(call, Codec::MuLaw.new, 0)
      end
      ...
      call.transport.client_addr = Socket::IPAddress.new(client_addr.address, sip_msg.sdp_header.media_port)
      # call.transport.client_addr = Socket::IPAddress.new("73.157.246.157", 8000)
            
      spawn { call.transport.listen }
